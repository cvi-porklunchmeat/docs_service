...
* fetch_and_download - Code for the application's Lambda function.
...
When testing locally, you can spin up a local S3 bucket via [localstack](https://localstack.cloud/):
...
Create the bucket:
```bash
awslocal s3api create-bucket --bucket dev-cloud-docs-service
```

You'll then be able to see your S3 bucket content by visiting:
* http://localhost:4566/dev-cloud-docs-service
...
```bash
document_service$ sam build --use-container
```
...
```bash
document_service$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
document_service$ sam local start-api
document_service$ curl http://localhost:3000/
```
...
```bash
document_service$ sam logs -n HelloWorldFunction --stack-name document_service --tail
```
...
```bash
document_service$ pip install -r tests/requirements.txt --user
document_service$ python -m pytest tests -v
```
...
```bash
sam delete --stack-name "document_service"
```
